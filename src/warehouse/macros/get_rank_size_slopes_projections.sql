...
WITH rank_size_slope_change AS (
    SELECT  country, 
            ...
            EXP(SUM(LN(1 + size_growth_slope)) OVER (PARTITION BY country, analysis_id, MOD(year, 10) ORDER BY year)) - 1 AS rank_size_slope_change
...
rank_size_slopes_projections AS (
    ...
    USING (country, analysis_id, mod_year)
)
SELECT * 
