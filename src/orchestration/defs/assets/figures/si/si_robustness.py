...
def _plot_size_growth_slope_usa_by_year_and_analysis_id(fig: plt.Figure, ax: plt.Axes, style_config: Dict[str, Any], df_size_vs_growth: pd.DataFrame, n_boots: int) -> Tuple[plt.Figure, plt.Axes]:
...
def _plot_slopes_usa_for_mixed_analysis(fig: plt.Figure, ax: plt.Axes, style_config: Dict[str, Any], df_size_vs_growth: pd.DataFrame, df_rank_vs_size: pd.DataFrame, n_boots: int) -> Tuple[plt.Figure, plt.Axes]:
    font_family = style_config['font_family']
    axis_font_size = style_config['axis_font_size']
    tick_font_size = style_config['tick_font_size']
    ...
    x_axis_inset = 'year'
    y_axis_inset = 'rank_size_slope'

    x_axis_inset_label = 'Year'
    y_axis_inset_label = 'Zipf exponent'
    ...
    ax.fill_between(size_growth_slope[x_axis], size_growth_slope['ci_low'], size_growth_slope['ci_high'], color=color, alpha=0.1)

    ax_inset = ax.inset_axes([0.6, 0.65, 0.3, 0.3])
    ax_inset.plot(rank_size_slope[x_axis_inset], rank_size_slope[y_axis_inset], color=color, linewidth=2, marker='o')
    ax_inset.fill_between(rank_size_slope[x_axis_inset], rank_size_slope['ci_low'], rank_size_slope['ci_high'], color=color, alpha=0.1)
...
def _plot_size_growth_curve_usa_by_analysis_id(fig: plt.Figure, ax: plt.Axes, style_config: Dict[str, Any], show_legend: bool, title: str, df_size_vs_growth_normalized: pd.DataFrame, df_average_growth: pd.DataFrame) -> Tuple[plt.Figure, plt.Axes]:
    title_font_size = style_config['title_font_size']
    font_family = style_config['font_family']
    axis_font_size = style_config['axis_font_size']
    tick_font_size = style_config['tick_font_size']

    map_analysis_id_to_label = {1: 50, 9: 100, 10: 200}
    ...
    for i, a in enumerate(analysis_ids):
        ...
        color = colors[i]
        ax.plot(x, average_log_growth_a + y, color=color, label=f'Urban threshold: {map_analysis_id_to_label[a]}', linewidth=2)
    ...
    if show_legend:
        ax.legend(fontsize=axis_font_size, frameon=False, bbox_to_anchor=(1.6, 1.25), ncol=3)
    sns.despine(ax=ax)
    return fig, ax
...
@dg.asset(
    deps=[TableNamesResource().names.usa.figures.usa_size_vs_growth(), TableNamesResource().names.usa.figures.usa_size_vs_growth_normalized(), TableNamesResource().names.usa.figures.usa_rank_vs_size(), TableNamesResource().names.usa.figures.usa_average_growth()],
    group_name="supplementary_information"
)
def usa_robustness_figure(context: dg.AssetExecutionContext, postgres: PostgresResource, tables: TableNamesResource) -> dg.MaterializeResult:
    ...
    n_boots = 10 # TODO: Change to 100

    analysis_ids = [analysis_id_50, analysis_id_100, analysis_id_200]
    size_vs_growth = pd.read_sql(f"SELECT * FROM {tables.names.usa.figures.usa_size_vs_growth()} WHERE analysis_id IN {tuple(analysis_ids)}", con=postgres.get_engine())
    ...
    rank_vs_size = pd.read_sql(f"SELECT * FROM {tables.names.usa.figures.usa_rank_vs_size()} WHERE analysis_id IN {tuple(analysis_ids)}", con=postgres.get_engine())
    ...
    e1 = [1850, 1860, 1870, 1880]
    e2 = [1900, 1910, 1920, 1930, 1940]
    e3 = [1990, 2000, 2010, 2020]
    mixed_size_vs_growth = pd.concat([size_vs_growth[(size_vs_growth['analysis_id'] == analysis_id_50) & (size_vs_growth['year'].isin(e1))], size_vs_growth[(size_vs_growth['analysis_id'] == analysis_id_100) & (size_vs_growth['year'].isin(e2))], size_vs_growth[(size_vs_growth['analysis_id'] == analysis_id_200) & (size_vs_growth['year'].isin(e3))]])
    mixed_rank_vs_size = pd.concat([rank_vs_size[(rank_vs_size['analysis_id'] == analysis_id_50) & (rank_vs_size['year'].isin(e1))], rank_vs_size[(rank_vs_size['analysis_id'] == analysis_id_100) & (rank_vs_size['year'].isin(e2))], rank_vs_size[(rank_vs_size['analysis_id'] == analysis_id_200) & (rank_vs_size['year'].isin(e3))]])
    ...
    ax5 = fig.add_subplot(gs1[4], sharey=ax4)
    ax6 = fig.add_subplot(gs1[5], sharey=ax4)

    _plot_size_growth_slope_usa_by_year_and_analysis_id(fig=fig, ax=ax1, style_config=style_config, df_size_vs_growth=size_vs_growth, n_boots=n_boots)
    _plot_rank_size_slope_usa_by_year_and_analysis_id(fig=fig, ax=ax2, style_config=style_config, df_size_vs_growth=rank_vs_size, n_boots=n_boots)
    _plot_slopes_usa_for_mixed_analysis(fig=fig, ax=ax3, style_config=style_config, df_size_vs_growth=mixed_size_vs_growth, df_rank_vs_size=mixed_rank_vs_size, n_boots=n_boots)

    epochs = ['1850-1880', '1900-1940', '1990-2020']
    axes = [ax4, ax5, ax6]
    for i, e in enumerate(epochs):
        ...
        if i == 1:
            _plot_size_growth_curve_usa_by_analysis_id(fig=fig, ax=axes[i], style_config=style_config, show_legend=True, title=e, df_size_vs_growth_normalized=size_vs_growth_normalized_e, df_average_growth=average_growth_e)
        else:
            _plot_size_growth_curve_usa_by_analysis_id(fig=fig, ax=axes[i], style_config=style_config, show_legend=False, title=e, df_size_vs_growth_normalized=size_vs_growth_normalized_e, df_average_growth=average_growth_e)
